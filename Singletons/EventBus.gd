...
func _init() -> void:
	...
	check_obj_right.connect(log_signal.bind(false, "check_obj_right"))
	check_obj_left.connect(log_signal.bind(false, "check_obj_left"))
	check_obj_under.connect(log_signal.bind(false, "check_obj_under"))
	check_just_fell.connect(log_signal.bind(false, "check_just_fell"))
	check_is_arrived.connect(log_signal.bind("check_is_arrived"))
	check_obj_right_response.connect(log_signal.bind("check_obj_right_response"))
	check_obj_left_response.connect(log_signal.bind("check_obj_left_response"))
	check_obj_under_response.connect(log_signal.bind("check_obj_under_response"))
